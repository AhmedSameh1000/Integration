...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20241202104351_addconditionstr')
BEGIN
    DROP TABLE [conditionFroms];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20241202104351_addconditionstr')
BEGIN
    DROP TABLE [ConditionTos];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[modules]') AND [c].[name] = N'ToUpdateFlagName');
    IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [modules] DROP CONSTRAINT [' + @var1 + '];');
    EXEC(N'UPDATE [modules] SET [ToUpdateFlagName] = N'''' WHERE [ToUpdateFlagName] IS NULL');
    ALTER TABLE [modules] ALTER COLUMN [ToUpdateFlagName] nvarchar(max) NOT NULL;
    ALTER TABLE [modules] ADD DEFAULT N'' FOR [ToUpdateFlagName];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[modules]') AND [c].[name] = N'ToInsertFlagName');
    IF @var2 IS NOT NULL EXEC(N'ALTER TABLE [modules] DROP CONSTRAINT [' + @var2 + '];');
    EXEC(N'UPDATE [modules] SET [ToInsertFlagName] = N'''' WHERE [ToInsertFlagName] IS NULL');
    ALTER TABLE [modules] ALTER COLUMN [ToInsertFlagName] nvarchar(max) NOT NULL;
    ALTER TABLE [modules] ADD DEFAULT N'' FOR [ToInsertFlagName];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[modules]') AND [c].[name] = N'LocalIdName');
    IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [modules] DROP CONSTRAINT [' + @var3 + '];');
    EXEC(N'UPDATE [modules] SET [LocalIdName] = N'''' WHERE [LocalIdName] IS NULL');
    ALTER TABLE [modules] ALTER COLUMN [LocalIdName] nvarchar(max) NOT NULL;
    ALTER TABLE [modules] ADD DEFAULT N'' FOR [LocalIdName];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[modules]') AND [c].[name] = N'FromUpdateFlagName');
    IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [modules] DROP CONSTRAINT [' + @var4 + '];');
    EXEC(N'UPDATE [modules] SET [FromUpdateFlagName] = N'''' WHERE [FromUpdateFlagName] IS NULL');
    ALTER TABLE [modules] ALTER COLUMN [FromUpdateFlagName] nvarchar(max) NOT NULL;
    ALTER TABLE [modules] ADD DEFAULT N'' FOR [FromUpdateFlagName];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[modules]') AND [c].[name] = N'FromInsertFlagName');
    IF @var5 IS NOT NULL EXEC(N'ALTER TABLE [modules] DROP CONSTRAINT [' + @var5 + '];');
    EXEC(N'UPDATE [modules] SET [FromInsertFlagName] = N'''' WHERE [FromInsertFlagName] IS NULL');
    ALTER TABLE [modules] ALTER COLUMN [FromInsertFlagName] nvarchar(max) NOT NULL;
    ALTER TABLE [modules] ADD DEFAULT N'' FOR [FromInsertFlagName];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[modules]') AND [c].[name] = N'CloudIdName');
    IF @var6 IS NOT NULL EXEC(N'ALTER TABLE [modules] DROP CONSTRAINT [' + @var6 + '];');
    EXEC(N'UPDATE [modules] SET [CloudIdName] = N'''' WHERE [CloudIdName] IS NULL');
    ALTER TABLE [modules] ALTER COLUMN [CloudIdName] nvarchar(max) NOT NULL;
    ALTER TABLE [modules] ADD DEFAULT N'' FOR [CloudIdName];
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20241202104351_addconditionstr')
BEGIN
    ALTER TABLE [modules] ADD [condition] nvarchar(max) NULL;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20241202104351_addconditionstr', N'7.0.20');
END;
GO
